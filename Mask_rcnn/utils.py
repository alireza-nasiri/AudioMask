...
def extract_bboxes(mask):
    ...
    for i in range(mask.shape[-1]):
        ...
        if horizontal_indicies.shape[0]:
            ...
            #x1 = max(0, x1 - int(w/2))
            #y1 = max(0, y1 - int(h/2))
            #y2= min(mask.shape[0], y2 + int(h/2))
            #x2 = min(mask.shape[1], x2 + int(w/2))
...
def compute_mask_IU(mask, masks):
    #print(mask.shape)
    ...
    # intersections and union
    intersections = np.dot(masks.T, mask)
...
def compute_mask_IU_1(mask, masks):
    #print(mask.shape)
    mask = np.reshape(mask > .5, (-1))
    masks = np.reshape(masks > .5, (-1, masks.shape[-1])).astype(np.float32)
    area1 = np.sum(mask)
...
def non_max_suppression_mask(masks, scores, threshold):
    ...
    while len(ixs) > 0:
        ...
        if len(ixs) == 1:
            break
        iu = compute_mask_IU_1(np.array([masks[:,:,i]]), masks[:,:,ixs[1:]])
        # Identify boxes with IoU over the threshold. This
        # returns indicies into ixs[1:], so add 1 to get
        # indicies into ixs.
        remove_ixs = np.where(iu > threshold)[0] + 1
        # Remove indicies of the picked and overlapped boxes.
        ixs = np.delete(ixs, remove_ixs)
        ixs = np.delete(ixs, 0)
    return np.array(pick, dtype=np.int32)
...
def minimize_mask(bbox, mask, mini_shape):
    ...
    for i in range(mask.shape[-1]):
        ...
        if m.size == 0:
            print(y1, y2, x1, x2)
            raise Exception(str(i) + ":Invalid bounding box with area of zero")
